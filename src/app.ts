...
var current:Array<number>= Array(7).fill(5);
const red = 1;
const yellow = 2;
const draw = 0;
const decode = {
...
const decodeLetter=(str:string)=> (str.charCodeAt(0) - "A".charCodeAt(0));
function createGrid(){
  grid = [];
...
function pushCard(color:number,pos:number):Array<number>{
  if(current[pos]!=-1){
    let cur = current[pos];
    grid[cur][pos]=color;
    current[pos]--;
...
function line(y:number,x:number,dy,dx){
  try {
...
function win(y:number,x:number){
...
function whoIsWinner(piecesPositionList:Array<string>){
